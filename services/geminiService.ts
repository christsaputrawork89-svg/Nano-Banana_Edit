...
`;
    ...
    parts.push({ inlineData: { mimeType: mainPart.mimeType, data: mainPart.data } });
    ...
    if (options.markedImageBase64) {
      ...
      parts.push({ inlineData: { mimeType: markedPart.mimeType, data: markedPart.data } });
    }
    ...
    if (options.referenceImages && options.referenceImages.length > 0) {
      for (const refFile of options.referenceImages) {
        ...
        parts.push({ inlineData: { mimeType: refPart.mimeType, data: refPart.data } });
      }
    }
